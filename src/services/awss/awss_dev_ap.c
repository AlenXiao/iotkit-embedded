...
int awss_dev_ap_start()
{
    ...
    ret = os_awss_open_ap(ssid, passwd, 100, 0);
...
{
#define AWSS_DEV_AP_SWITCHA_RSP_LEN (256)
    ...
    do {
        awss_build_dev_info(AWSS_NOTIFY_DEV_BIND_TOKEN, dev_info, AWSS_DEV_AP_SWITCHA_RSP_LEN);
        ...
        if (str_len < (PLATFORM_MAX_PASSWD_LEN * 2) - 1) {
            ...
            aes_decrypt_string(encoded, passwd, str_len, os_get_encrypt_type(), 0); // 64bytes=2x32bytes
        } else {
